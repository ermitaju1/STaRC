...
class DenseVideoCaptioning_Dataset(Dataset):
    ...
    def _get_video(self, video_id, video_id_full=None):
        if self.features is not None:
            # Try full video_id first, then fallback to short version
            if video_id_full is not None and video_id_full in self.features:
                video = self.features[video_id_full].float()
            elif video_id in self.features:
                video = self.features[video_id].float()
            else:
                raise AssertionError(f"video_id {video_id} (full: {video_id_full}) not found in features")
        else:
    ...
    def __getitem__(self, idx):
        ...
        video = self._get_video(video_id[-11:], video_id_full=video_id)
...
def build_densevideocaptioning_dataset(dataset_name, split, args, tokenizer):
    if dataset_name == "youcook":
        ...
        if split == "train":
            json_path = args.train_caption_file
        elif split == "val":
            json_path = args.val_caption_file
        else:
            raise NotImplementedError
        features_path = args.visual_feature_folder[0] if isinstance(args.visual_feature_folder, list) else args.visual_feature_folder
        subtitles_path = getattr(args, 'anet_subtitles_path', None)
